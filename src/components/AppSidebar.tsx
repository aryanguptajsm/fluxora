...
export function AppSidebar({
  ...
  const togglePin = (id: number) => {
    setPinnedItems(prev => prev.includes(id) ? prev.filter(i => i !== id) : [...prev, id]);
  };
  const filteredHistory = historyItems.filter(item => item.title.toLowerCase().includes(searchQuery.toLowerCase()) || item.prompt.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  return <Sidebar className="border-r border-sidebar-border bg-sidebar" collapsible="icon">
      ...
      <SidebarContent>
        <div className="p-3">
          <Button className={`w-full bg-gradient-primary hover:opacity-90 transition-all shadow-glow ${isCollapsed ? 'px-0 justify-center' : ''}`} onClick={onNewChat}>
          ...
          <SidebarGroup>
            <SidebarGroupLabel asChild>
              <CollapsibleTrigger className={`w-full flex items-center justify-between text-xs uppercase tracking-wider text-muted-foreground px-3 hover:bg-sidebar-accent rounded-md transition-colors ${isCollapsed ? 'justify-center' : ''}`}>
                ...
                {!isCollapsed && <ChevronDown className={`h-4 w-4 flex-shrink-0 transition-transform ${historyOpen ? 'rotate-180' : ''}`} />}
              </CollapsibleTrigger>
            </SidebarGroupLabel>
            
            <CollapsibleContent>
              <SidebarGroupContent>
                {!isCollapsed && historyItems.length > 0 && <div className="px-3 py-2">
                    <div className="relative">
                      <Search className="absolute left-2 top-1/2 -translate-y-1/2 h-3 w-3 text-muted-foreground" />
                      <Input placeholder="Search history..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="h-8 pl-7 text-xs bg-sidebar-accent/50 border-sidebar-border" />
                    </div>
                  </div>}
                
                <ScrollArea className="h-[350px] will-change-scroll">
                  {historyItems.length === 0 ? <div className="px-3 py-8 text-center">
                    ...
                    return <SidebarMenuItem key={item.id}>
                            <div className={`group/item hover:bg-sidebar-accent transition-all relative ${currentHistoryId === item.id ? 'bg-sidebar-accent border-l-2 border-primary' : ''}`}>
                              ...
                              {!isCollapsed && <div className="absolute right-2 top-1/2 -translate-y-1/2 opacity-0 group-hover/item:opacity-100 transition-opacity flex gap-1">
                            ...
                            togglePin(item.id);
                                  ...
                                  <Button variant="ghost" size="icon" className="h-7 w-7 hover:bg-destructive/20 hover:text-destructive" onClick={e => {
                            e.stopPropagation();
      ...
      <SidebarFooter className="border-t border-sidebar-border p-4">
        {user ? <Button variant="outline" className={`w-full bg-destructive/10 hover:bg-destructive/20 text-destructive border-destructive/20 ${isCollapsed ? 'px-0 justify-center' : ''}`} onClick={handleSignOut}>
            <LogOut className={`h-4 w-4 ${!isCollapsed ? 'mr-2' : ''}`} />
            {!isCollapsed && <span className="font-medium">Sign Out</span>}
          </Button> : <Button variant="outline" className={`w-full bg-primary/10 hover:bg-primary/20 text-primary border-primary/20 ${isCollapsed ? 'px-0 justify-center' : ''}`} onClick={handleSignIn}>
